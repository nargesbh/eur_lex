...
def build_dolma_document(pdf_orig_path, page_results):
    if not page_results:
        logger.info(f"No page results for {pdf_orig_path}")
        return None
    ...
    for page_result in page_results:
        pr = page_result.response
        page_data = {
            "page_number": page_result.page_num,
            "primary_language": pr.primary_language,
            "is_rotation_valid": pr.is_rotation_valid,
            "rotation_correction": pr.rotation_correction,
            "is_table": pr.is_table,
            "is_diagram": pr.is_diagram,
            "natural_text": pr.natural_text,
        }
...
async def worker(args, work_queue: WorkQueue, semaphore, worker_id):
    ...
    timing_csv = "/ltstorage/shares/datasets/eu/category15/json_timing.csv"
    if not os.path.exists(timing_csv):
        with open(timing_csv, "w") as f:
            f.write("pdf,time_sec\n")
    ...
    while True:
        ...
        try:
            ...
            for task, pdf in zip(dolma_tasks, work_item.work_paths):
                try:
                ...
                with open(timing_csv, "a") as f:
                    writer = csv.writer(f)
                    writer.writerow([pdf, round(end_time - start_time, 2)])
