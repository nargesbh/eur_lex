...
def main():
    ...
    # Prepare output timing CSV
    csv_path = args.out / "timing.csv"
    csv_file = open(csv_path, "w", newline="", encoding="utf-8")
    csv_writer = csv.DictWriter(csv_file, fieldnames=["pdf_file", "seconds"])
    ...
    for pdf in args.pdf:
        ...
        if pdf.is_file():
            ...
            all_pdfs.extend(list(pdf.rglob("*.pdf")))
    logging.info(f"Found {len(all_pdfs)} PDFs to process.")

    # Top-level tqdm for all PDFs
    for pdf in tqdm(all_pdfs, desc="Processing PDFs", unit="pdf"):
        start_time = time.time()

        out_path = args.out / pdf.with_suffix(".mmd").name
        if out_path.exists() and not args.recompute:
            logging.info(f"Skipping {pdf.name}, already computed. Use --recompute to overwrite.")
            continue
        ...
        # Inner tqdm for pages
        for i, (sample, is_last_page) in enumerate(tqdm(dataloader, leave=False, desc=f"{pdf.name}")):
        ...
        csv_writer.writerow({"pdf_file": str(pdf), "seconds": duration})
        csv_file.flush()
